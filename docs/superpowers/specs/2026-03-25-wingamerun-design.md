...
│       ├── WhiskyWine/
│       │   └── WineInstaller.swift  # 引擎下载和安装
│       ├── Steam/
│       │   └── SteamIntegration.swift  # 新增：Steam 集成
│       ├── PE/                  # Windows PE 文件解析
│       └── Extensions/
│
├── WinGameCmd/                  # CLI 工具
...
├── Containers/
│   └── com.wingamerun.app/
│       ├── BottleVM.plist         # Bottle 路径索引
│       └── Bottles/               # 默认 Bottles 位置
│           └── Default/
...
原 Bourbon 从自己的 GitHub Release 下载预编译的 Wine tarball。WinGameRun 需要：

```swift
// WinGameKit/Sources/WinGameKit/WhiskyWine/WineInstaller.swift
struct WineInstaller {
    // 改为从 Gcenx 的公共 Wine 构建下载
    static let wineReleasesAPI = "https://api.github.com/repos/Gcenx/wine-crossover/releases/latest"
...
func isD3DMetalAvailable() -> Bool {
    // 检查 GPTK 安装路径
    // 或检查 /usr/local/opt/game-porting-toolkit/ 是否存在
}
...
public class SteamIntegration {
    ...
    /// 解析已安装游戏列表（appId, name, installDir）
...
|------|---------|
| Wine 未安装 | 自动跳转安装向导 |
| D3DMetal 未安装 | 弹窗引导安装 GPTK，提供降级选项 |
...
| 层级 | 测试内容 | 工具 |
|------|---------|------|
| 单元测试 | PE 解析、配置序列化、路径管理 | XCTest |
| 集成测试 | Wine 进程启动、Bottle CRUD | XCTest |
| UI 测试 | 首次运行向导、添加游戏、启动 | XCUITest |
...
- [Gcenx/wine-crossover](https://github.com/Gcenx/wine-crossover) — Wine 引擎来源
