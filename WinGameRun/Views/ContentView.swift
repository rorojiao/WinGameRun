        ...
        .task {
            ...
            if !bottleVM.bottles.isEmpty {
                if let bottle = bottleVM.bottles.first(where: { $0.url == selectedBottleURL && $0.isAvailable }) {
                    selected = bottle.url
                } else if let firstBottle = bottleVM.bottles.first {
                    selected = firstBottle.url
                }
            }
            ...
            let task = Task.detached {
                return await WineInstaller.shouldUpdateWine()
            }
            let updateInfo = await task.value
            if updateInfo.0 {
                let alert = NSAlert()
                alert.messageText = String(localized: "update.wine.title")
                alert.informativeText = String(format: String(localized: "update.wine.description"),
                                               String(WineInstaller.wineVersion()
                                                      ?? SemanticVersion(0, 0, 0)),
                                               String(updateInfo.1))
                alert.alertStyle = .warning
                alert.addButton(withTitle: String(localized: "update.wine.update"))
